...
for line in text_data.split("\n"):
    ...
    for date in date_match:
        ...
        for task in task_matches:
            # タスクごとに詳細部分のパターンを作成
            detail_pattern = rf"{task} - (.*?)(?= - |。)"
            start_pos = line.find(f"{date}：{task} - ")
            if start_pos != -1:
                # タスクの詳細部分を抽出
                detail_text = line[start_pos:]
                detail_matches = re.findall(detail_pattern, detail_text, re.DOTALL)
                tasks[date][task] = detail_matches
