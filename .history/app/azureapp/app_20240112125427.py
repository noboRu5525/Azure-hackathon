    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "私は個人プロジェクトとして食材管理アプリケーションの開発に取り組んでいます。このアプリは、カメラで撮影した画像を使って冷蔵庫の中の食材を検出し管理し、データベースにある食材からレシピを生成し、購入すべき相性の良い食材を提案します。プロジェクトの期間は120日です。私のスキルにはPython、HTML、CSS、JavaScriptがあり、Firebaseを使用する予定です。効率よく目標を達成するためのタスク計画と学習計画を作成してほしいです。計画は日単位で、必要なプログラミング言語やツール（APIなど）の詳細を含めてください。必要に応じて追加の言語やツールを計画に組み込んでください。すでに習得している言語の学習は除外してください。個人プロジェクトのため、柔軟性を持たせた計画を立てて、指定された120日間を最大限に活用してください。"}
            ]
        )
