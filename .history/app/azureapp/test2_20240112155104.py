...
for line in text_data.split("\n"):
    ...
    for date in date_match:
        ...
        for task in task_matches:
            ...
            detail_pattern = rf"{date}：{task} - (.*?)[\d+日目]"
            detail_matches = re.findall(detail_pattern, line)
            detail_line = detail_matches[0].split("-")
            print(detail_line)

            # タスクと詳細の数を辞書に格納
            tasks[date][task] = {'詳細数': detail_count, '詳細': detail_matches}
...
for date, task_info in tasks.items():
    ...
    for task, details in task_info.items():
        print(f"  タスク: {task}, 詳細数: {details['詳細数']}")
        for detail in details['詳細']:
            print(f"    詳細: {detail.strip()}")
