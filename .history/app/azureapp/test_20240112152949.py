...
for line in text_data.split("\n"):
    ...
    for date in date_match:
        ...
        task_match = re.findall(task_pattern, line)
        #print(task_match)
        tasks[date] = task_match   
        for task in task_match:
            # タスクの詳細を抽出するパターン
            detail_pattern = rf"(?<={date}：{task} - )(.*?)(?=( - |。))"
            detail_matches = re.findall(detail_pattern, line, re.DOTALL)
            print(detail_matches)
        """
        detail_pattern = rf"(?<={date}：{task_match[0]} - )(.*?)(?=( - |。))"
        detail_match = re.findall(detail_pattern, line, re.DOTALL)
        print(detail_match)"""
