...
def save_data():
    try:
        ...
        # タスク実行情報を挿入
        insert_query = """
        INSERT INTO task_executions (task_id, execution_date, execution_time, task_progress, user_memo)
        VALUES (%s, %s, %s, %s, %s)
        """
...
def stats():
    ...
    cursor.execute('''
        SELECT p.id AS project_id, t.id AS task_id, SUM(te.execution_time) AS total_execution_time
        FROM projects p
        JOIN tasks t ON p.id = t.plan_id
        LEFT JOIN task_executions te ON t.id = te.task_id
        WHERE p.user_id = %s
        GROUP BY p.id, t.id
    ''', (user_id,))
    project_task_data = cursor.fetchall()

    return render_template('activity.html', project_task_data=project_task_data)
