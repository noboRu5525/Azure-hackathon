...
for line in text_data.split("\n"):
    ...
    for date in date_match:
        ...
        for task in task_matches:
            # タスクの開始位置を特定
            task_start_pos = line.find(f"{date}：{task} - ") + len(f"{date}：{task} - ")

            # 次のタスクの開始位置を探す
            next_task_match = re.search(task_pattern, line[task_start_pos:])
            if next_task_match:
                task_end_pos = task_start_pos + next_task_match.start()
            else:
                task_end_pos = len(line)

            # タスクの詳細部分を抽出
            detail_text = line[task_start_pos:task_end_pos]
            detail_matches = re.findall(r"(.*?)(?= - |。)", detail_text, re.DOTALL)
            tasks[date][task] = [detail.strip() for detail in detail_matches if detail.strip()]
