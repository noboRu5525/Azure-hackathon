...
def openai():
    ...
    #テスト用
    makedata = []
    makedata.append(category)
    makedata.append(systemName)
    makedata.append(makeDay)
    makedata.append(functions)
    makedata.append(languages)
    makedata.append(tools)

    # 各機能を「。」で区切って結合
    functions_str = '<br>'.join(functions)
    ...
    return jsonify({'message': functions_str })
    #return res
