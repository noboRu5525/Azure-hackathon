...
def stats():
    ...
    for row in fetched_data:  # データベースから取得したデータをループ処理
        execution_date = row['execution_date']
        execution_time = row['execution_time']
        if execution_date:  # NULLでない場合のみ処理
            date = execution_date.date()
            activities_by_day[date] += execution_time

    # 日別の活動時間を取得
    daily_activity_totals = dict(activities_by_day)
    return
